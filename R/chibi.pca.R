...
chibi.pca<-function(list_ohpca=NULL,col_val=NULL,shape_val=NULL,comp_a="PC1",comp_b="PC2",mypch=21,
  ...
  Map_pca<-list_ohpca$Map_pca
  myvar=list_ohpca$variance_explained
  if(lines_zero == FALSE) {
    if(is.null(shape_val)){
      p <- ggplot(data = Map_pca,aes_string(x = comp_a,y = comp_b))+
      ...
      p <- ggplot(data = Map_pca,aes_string(x = comp_a,y = comp_b,shape=shape_val))+

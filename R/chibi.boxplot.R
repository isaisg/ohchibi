...
chibi.boxplot<-function(Map=Map,x_val=NULL,y_val=NULL,col_val=NULL,shape_val=NULL,style="full",mypch_point=21,size_point=8,alpha_point=1,stroke_point=0.5,size_boxplot=2){
  #Check the overall style
  if(style == "full"){
    p <- ggplot(Map, aes_string(x = x_val, y = y_val,fill = col_val)) + 
      geom_boxplot(color="#414141", outlier.colour = NA, position = position_dodge(width = 0.9), size=size_boxplot) +  
    ...
    if(is.null(shape_val)){
      ...
      p<-p + geom_point(position = position_jitterdodge(dodge.width = 0.9, jitter.width = 0.1), 
                        size = size_point,stroke=stroke_point,alpha=alpha_point,aes_string(x=x_val,y=y_val,col=col_val,fill=col_val,shape=shape_val),inherit.aes = F)

...
chibi.boxplot<-function(Map=Map,x_val=NULL,y_val=NULL,col_val=NULL,shape_val=NULL,style="mix",
  ...
  #Facetting just implemented in the mix style
  if((style == "full" | style == "open") & ! is.null(facet_formula)){
    stop("ERROR: facet_formula is only implemented in the mix style. Use the mix style to call for it",call.=TRUE)
  ...
  if(style == "full"){
    if(is.null(col_val)){
      ...
      if(is.null(col_val)){
        ...
        #This is the second style of open that is all the boxplot colored and dots in a single color
      ...
      if(is.null(facet_formula)){
          ...
          if(lfacet == 1){
            ...
            if(median_colored_as_points == FALSE){
              ...
              #Draw the automatic vline
              #Determine the levels per facet_val
              #Use map melted instead jsut create another columns with smaller combinations meaning only the facceting ones
              num_facet_one <- length(levels(Map[,which(colnames(map_melted)==facet_vals[1])]))
              num_facet_two <- length(levels(Map[,which(colnames(map_melted)==facet_vals[2])]))
              #This may still fail if missing values
              num_facet <- num_facet_one*num_facet_two
              num_x <- length(levels(Map[,which(colnames(Map)==x_val)]))
              myrep <- num_facet*num_x
              iter <- myrep/num_x
              tot_interval <- 1:myrep
